...
PROG=kgsearch
PROG_JAR="/work/jar/kgsearch.jar"
...
PID=$(ps -ef|grep kgsearch|grep -v grep|awk '{print $2}')
if [ -z $PID ]; then
	echo "process kgsearch not exist"
	exit
else
	echo "process id: $PID"
	kill -9 ${PID}
	echo "process kgsearch killed"
fi

nohup java  $JAVA_OPTS -jar $PROG_JAR > $LOGDIR/kgsearch.log 2>&1
