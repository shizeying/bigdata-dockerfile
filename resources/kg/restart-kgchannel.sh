...
PROG=kgchannel
PROG_JAR="/work/jar/kgchannel.jar"
...
PID=$(ps -ef|grep kgchannel|grep -v grep|awk '{print $2}')
if [ -z $PID ]; then
	echo "process kgchannel not exist"
	exit
else
	echo "process id: $PID"
	kill -9 ${PID}
	echo "process kgchannel killed"
fi

nohup java  $JAVA_OPTS -jar $PROG_JAR > $LOGDIR/kgchannel.log 2>&1
